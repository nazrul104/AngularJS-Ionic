...
class ManageUser extends REST
{
	...
	public function Login()
	{
		...
		$status=1;
		$query="SELECT a.id as userid,a.user_group_id,a.email,b.first_name,b.sur_name,b.last_name,b.postcode,b.user_id,b.mobile_no,b.address1, b.address2, b.date_of_anniversery, b.date_of_birth, b.telephone_no, b.town, b.region FROM user a, user_profile b WHERE a.email='".$username."'AND a.password='".$pass."' AND a.id=b.user_id AND a.status='".$status."'";
		$result = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
		if($result->num_rows > 0) 
		{
			$row = $result->fetch_assoc();
			$success = array('status' => "Success","UserDetails"=>array("userid"=>$row['userid'],"user_group_id"=>$row['user_group_id'],"email"=>$row['email'],"first_name"=>$row['first_name'],"sur_name"=>$row['sur_name'],"last_name"=>$row['last_name'],"postcode"=>$row['postcode'],"mobile_no"=>$row['mobile_no'],"address1"=>$row['address1'],"address2"=>$row['address2'],"date_of_birth"=>(!empty($row['date_of_birth']))? $row['date_of_birth'] : "","date_of_anniversery"=>(!empty($row['date_of_anniversery']))? $row['date_of_anniversery'] : "","telephone_no"=>$row['telephone_no'],"town"=>$row['town'], "country"=>$row['region']));
			echo json_encode($success);
		}
		else
		{
			$failed = array('status' => "Failed", "UserDetails"=>array('Message'=>'Invalid Username or Password.'));
		...
		if ($r)
		 {
			...
			$subject = "Account Confirmation from ChefOnline";			
			$message_body = "Welcome $fname, You are successfully registered in ChefOnline.";
			self::AccountConfirmationEmail($email, $subject, $message_body);
	...
	private function SendEmail($to,$rid)
	{
		...
		$message.="<br>Best Regards<br> Smart Restaurent Solution Team.";
	...
	private function AccountConfirmationEmail($to, $sub, $msg)
	{
		$headers = 'From: ChefOnline info@chefonline.co.uk' . "\r\n" ;
		$headers .='Reply-To: '. $to . "\r\n" ;
		// $headers .= "X-Priority: 3\r\n";
		...
		if($result->num_rows > 0)
		 {
			while($row = $result->fetch_assoc()) 
			{
				...
				$arr['date_of_birth'] = (!empty($row['date_of_birth']))? $row['date_of_birth'] : ''; 
				$arr['date_of_anniversery'] = (!empty($row['date_of_anniversery']))? $row['date_of_anniversery'] : ''; 
				$arr['postcode'] = $row['postcode']; 
				$arr['town'] = $row['town']; 
				$arr['country'] = $row['region']; 
			}
		}		
		return $arr;
	}
