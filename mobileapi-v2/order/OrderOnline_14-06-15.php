...
class OrderOnline extends REST
{
    ...
    if ($_REQUEST['searchText']!="")
     {
      ...
      {
        $query_where .= " AND b.name = '".$_REQUEST['ordertype']."' AND b.status = 1";
      }
    
      $query="select DISTINCT a.id as rest_id, a.* from restaurant a INNER JOIN restaurant_order_policy b ON a.id = b.restaurant_id $query_where AND a.is_policy_set = 1";
      $this->mysqli->set_charset("utf8");
      $result = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
  ...
  {
      $query_where=" WHERE a.is_policy_set = 1 ORDER BY RAND() LIMIT 10";

      $query="select a.id as rest_id,a.logo,a.restaurant_name,a.address1,a.address2,a.city,a.domain,a.latitude,a.longitude,a.postcode,a.business_tel, a.accept_reservation FROM restaurant a $query_where";
      $this->mysqli->set_charset("utf8");
      $result = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
      $arr["app"] = array();
      if($result->num_rows > 0) 
      {
        while($row = $result->fetch_assoc()) 
        {
          ...
          if(count(self::getOfferInformation($row['rest_id'])) > 0)
          {
            $resturent=array();
            $resturent['rest_id']=$row['rest_id'];
            $resturent['logo']=$row['logo'];
