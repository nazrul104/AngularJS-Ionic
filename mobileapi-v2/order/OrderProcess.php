...
class OrderProcess extends REST
{
	...
	public function updateOrder()
	{
		...
		$restaurant_id =  mysqli_real_escape_string($this->mysqli,$_REQUEST['rest_id']);
		//$order_id =  mysqli_real_escape_string($this->mysqli,$_POST['order_id']);
		$payment_status =  mysqli_real_escape_string($this->mysqli,$_REQUEST['payment_status']);
		$paypal_transaction_id = mysqli_real_escape_string($this->mysqli,$_REQUEST['transaction_id']);
		 	...
		 	$query ="UPDATE restaurant_order SET payment_status='$payment_status' WHERE restaurant_id='".$restaurant_id."' and paypal_transaction_id='$paypal_transaction_id'";

			$rst=$this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
			
			if ($rst) 
			{
				$success = array('status' => "Success", "msg" => "Order update done!");
				echo json_encode($success);
			}
			else  {
	...
	function CheckPaypalOrderStatusForIonic()
	{
		$restaurant_id =  mysqli_real_escape_string($this->mysqli,$_REQUEST['rest_id']);
		//$payment_status =  mysqli_real_escape_string($this->mysqli,$_REQUEST['payment_status']);
		$paypal_transaction_id = mysqli_real_escape_string($this->mysqli,$_REQUEST['transaction_id']);
		 	$query ="SELECT *FROM restaurant_order  WHERE restaurant_id='".$restaurant_id."' AND paypal_transaction_id='$paypal_transaction_id'";
