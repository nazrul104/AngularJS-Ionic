...
class OrderOnline extends REST
{
      ...
      if($result->num_rows > 0) 
      {
        while($row = $result->fetch_assoc())
        {
          if($row['parent_dish_id']==0)
          {
            ...
            $dish['Dish_Acm_Price'] = $row['acm_price'];
        ...
        if($result->num_rows > 0)
         {
          ...
          while($row = $result->fetch_assoc()) 
          {
            ...
            $resturent['description']= strip_tags($row['description']);
  ...
  public function getresturentInformation()
  {
    ...
    $query_where=" WHERE a.is_policy_set = 1 AND a.id NOT IN (16)";
    if(isset($_REQUEST['postcode']))
    {
      ...
      $query="select a.id as rest_id,a.logo,a.slider_image,a.restaurant_name,a.address1,a.address2,a.city,a.domain,a.latitude,a.longitude,a.postcode,a.business_tel, a.accept_reservation FROM restaurant a $query_where LIMIT 10";
      $this->mysqli->set_charset("utf8");
      $result = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
      $arr["app"] = array();
      if($result->num_rows > 0) 
      {
        while($row = $result->fetch_assoc()) 
        {
          ...
          $resturent['address1']= str_replace('<br/>', " ", $row['address1']);
    ...
    if ($_REQUEST['searchText']!="")
     {
      ...
      $query_where =" WHERE a.postcode LIKE '$code%' OR a.restaurant_name LIKE '%$code%' AND a.id NOT IN (16)";
      ...
      if($result->num_rows > 0) 
      {
        while($row = $result->fetch_assoc()) 
        {
          if($_REQUEST['ordertype'] == "Reservation")
          {
            $cond = ($row['is_policy_set']==1 && $row['accept_reservation']==1);
          }
          else
          {
            $cond = ($row['is_policy_set'] == 1);
          }
  ...
  public function getPreviousOrders(){
    ...
    if(!empty($userid))
    {
      ...
      $query = "select a.id as order_id, a.user_id, CASE WHEN a.grand_total THEN a.grand_total ELSE '0.00' END as grand_total, a.created_at as order_date, CASE a.payment_method WHEN 0 THEN 'Cash' WHEN 1 THEN 'PayPal' WHEN 2 THEN 'PayOverPhone' ELSE 'Cash' END as 'payment_method', CASE a.payment_status WHEN 1 THEN 'Confirmed' ELSE 'Not Confirmed' END as payment_status, a.paypal_transaction_id, b.id as rest_id, b.restaurant_name, b.logo as restaurant_logo, b.address1,b.address2,b.city,b.domain,b.latitude,b.longitude,b.postcode,b.business_tel, b.accept_reservation from restaurant_order a inner join restaurant b on a.restaurant_id = b.id where a.user_id = '$userid' AND b.id NOT IN (16) ORDER BY a.id DESC";

      // $query = "SELECT * FROM v_customerpreviousorders WHERE user_id = '$userid'";
      ...
      if($result->num_rows > 0) 
      {
        ...
        while($row = $result->fetch_assoc()) 
        {
          ...
          $order['paypal_transaction_id'] = $row['paypal_transaction_id'];
          $order['address1']=str_replace('<br/>','',$row['address1']);;
          $order['address2']=str_replace('<br/>','',$row['address2']);;
          $order['city']=$row['city'];
        ...
        $arr['status'] = 'Success';
        $arr['msg'] = "You have ".$count.". previous orders";
        echo json_encode($arr);
      }
      else
      {
  ...
  {
    $order_id = mysqli_real_escape_string($this->mysqli, $_REQUEST['order_id']);
    $query_where ="";
    $arr["order"] = array();
  ...
  {
      $query_where=" WHERE a.is_policy_set = 1 AND b.status = 1 AND a.id NOT IN (16) GROUP BY b.restaurant_id HAVING COUNT(b.restaurant_id) > 0 ORDER BY RAND() LIMIT 10";
    ...
    header('Access-Control-Allow-Methods: POST, GET, OPTIONS, DELETE, PUT');    
     ...
     if($result->num_rows > 0)
     {
       while($row = $result->fetch_assoc()) 
       {
         ...
         $arr['restuarent_schedule']=self::getResturentSchedule($rest_id);         
